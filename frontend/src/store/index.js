...
axios.defaults.baseURL = `http://${process.env.NODE_ENV === 'development' ? 'localhost' : 'rollinitiative.app'}:${process.env.API_PORT || 3000}/`;
...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        updateActorInTracker(state, newActor) {
            if (state.currentInitiativeTracker.actors.find(actor => actor.id === newActor.id)) {
                let index = state.currentInitiativeTracker.actors.indexOf(state.currentInitiativeTracker.actors.find(actor => actor.id === newActor.id));
                state.currentInitiativeTracker.actors.splice(index, 1, newActor);
                localStorage.setItem('currentInitiativeTracker', JSON.stringify(state.currentInitiativeTracker));
            }
        }
    },
    actions: {
        ...
        updateActorInTracker(context, actor) {
            context.commit('updateActorInTracker', actor);
        },
        async getUserData() {
        ...
        async verifyAccount(context, token) {
            ...
            if(response && response.status === 200) {
                ...
                context.commit('updateToken', jwt);
                context.commit('signIn', true);
            }
        },
        ...
        async sendPassReset(context, email) {
            const response = await axios.post('/user/requestpassreset', {email});
